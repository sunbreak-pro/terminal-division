...
// テスト側から Panel.onResize を発火させるためのレジストリ
type PanelResizeHandler = (
  size: { asPercentage: number; inPixels: number },
  id: string | number | undefined,
  prev: { asPercentage: number; inPixels: number } | undefined,
) => void;
const panelOnResizeMap = new Map<string, PanelResizeHandler>();
...
vi.mock("react-resizable-panels", () => ({
  ...
  Panel: ({
    ...
    if (onResize) panelOnResizeMap.set(id, onResize);
    return <div data-testid={`panel-${id}`}>{children}</div>;
  },
  Separator: ({ id }: { id: string }) => (
    <div data-testid={`separator-${id}`} />
  ),
}));
...
describe("SplitContainer", () => {
  ...
  it("renders horizontal split", () => {
    ...
    // セパレーターがある
    expect(
      screen.getByTestId("separator-handle-split-1-0"),
    ).toBeInTheDocument();
  });
  ...
  it("renders nested splits", () => {
    ...
    const terminal1: TerminalPane = {
      id: "terminal-1",
      parentId: "split-root",
    };
    const terminal2: TerminalPane = {
      id: "terminal-2",
      parentId: "split-nested",
    };
    const terminal3: TerminalPane = {
      id: "terminal-3",
      parentId: "split-nested",
    };
  ...
  it("Panel.onResize triggers fit + pty.resize for leaf pane only", () => {
    const nodes = new Map<string, LayoutNode>();
    const splitNode: SplitNode = {
      id: "split-1",
      type: "split",
    ...
    nodes.set("split-1", splitNode);
    nodes.set("terminal-1", { id: "terminal-1", parentId: "split-1" });
    nodes.set("terminal-2", { id: "terminal-2", parentId: "split-1" });
    ...
    // 葉ペインの onResize が登録されている
    const onResize = panelOnResizeMap.get("terminal-1");
    expect(onResize).toBeDefined();

    onResize!({ asPercentage: 50, inPixels: 400 }, "terminal-1", {
      asPercentage: 60,
      inPixels: 480,
    });
  ...
  it("Panel.onResize ignored when fit() returns null (size unchanged)", () => {
    const nodes = new Map<string, LayoutNode>();
    nodes.set("terminal-only", { id: "terminal-only", parentId: null });

    vi.mocked(terminalStore.useRootId).mockReturnValue("terminal-only");
    vi.mocked(terminalStore.useNodes).mockReturnValue(nodes);
