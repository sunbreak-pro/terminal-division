...
## T3-4: セッション永続化（タブ / ペイン構成の復元）

### Purpose

アプリ再起動後も前回のペイン構成と CWD を復元する。
...
- **含む候補**: レイアウトツリーの JSON 永続化、起動時復元、CWD のみ復元
- **含まない**: 実行中プロセスの復元（PTY プロセスは永続化不可）

### 判断材料

- ユーザー導線として価値は高いが、実装は軽量（Zustand persist ミドルウェア）
- 既存の Dock「最近のディレクトリ」である程度カバー済み
