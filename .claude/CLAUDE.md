...
- **T2-10**: カスタマイズ拡張（`AppSettings` に `terminal` / `editor` / `chat` / `general` を追加して永続化。タブ「ターミナル」「エディタ」「一般」で UI 提供。ターミナル: フォントサイズ / ファミリー / 行間 / カーソルスタイル + blink / scrollback / Bell（none/visual/sound）/ 単語区切り / デフォルトシェル / デフォルト CWD。エディタ: フォントサイズ / ファミリー / softWrap。チャット: フォントサイズ（メッセージバブル + 入力欄）。一般: セッション復元 ON/OFF / PTY 異常終了通知 / アプリ全体ズーム倍率 (`appZoomFactor`)。フォントズーム `Cmd+=` / `Cmd+-` / `Cmd+0` はアクティブペインの viewMode に応じて分岐し、cli → `terminal.fontSize` / md → `editor.fontSize` / chat → `chat.fontSize` のグローバル設定を直接更新（Settings UI のスライダーと常に同期）。アプリ全体ズームは preload 経由 `webFrame.setZoomFactor` で適用（50%〜200%、ターミナル個別ズームと独立して併用可能）。ペインタイトル: SubHeader をダブルクリックで rename、空文字列確定で自動表示（CWD 由来）に戻る）
